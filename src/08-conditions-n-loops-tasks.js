...
function getDigitalRoot(num) {
  return num < 10 ? num : getDigitalRoot(+num.toString().split('').reduce((acc, val) => acc + +val, 0));
}
 ...
 */
function getCommonDirectoryPath(rawPaths) {
  const paths = rawPaths.map((val) => val.split('/'));
  const res = paths[0].filter((v1, i) => paths.map((v2) => v2[i]).every((item) => item === v1));
  return [...res, ''].join('/');
}
